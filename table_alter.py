...
def generate_combined_schema(table_schema, data_dictionary, sample_data, df, table_name):
    ...
    for col_info in table_schema:
        ...
        # Safely check for nulls only if the column exists in the original DataFrame.
...
def execute_alter_table_query(query: str):
    ...
    # Regenerate the Data Dictionary only after the ALTER TABLE query is executed.
    # This block runs after user confirmation and execution of the ALTER TABLE query.
    if data_dictionary_toggle:
...
######################################
# Initialize pending query state for modification and ALTER TABLE queries
...
if st.button("Submit") and user_question:
    ...
    # Check if the generated query is an ALTER TABLE query.
    if re.match(r'^\s*ALTER\s+TABLE', query_generated, re.IGNORECASE):
        if st.session_state.get("safe_mode", True):
            ...
            # Instead of executing immediately, store it as a pending ALTER TABLE query
            st.session_state["pending_alter_query"] = query_generated
            st.session_state["alter_confirmed"] = False

    # Else if it's a modification query (INSERT/UPDATE/DELETE) and safe mode is off,
    # store it as pending for confirmation.
...
if st.session_state.get("pending_alter_query"):
    ...
    if not st.session_state.get("safe_mode", True):
        ...
        if not st.session_state.get("alter_confirmed"):
            if st.button("Confirm Execution of ALTER TABLE Query", key="confirm_alter_query"):
                st.session_state["alter_confirmed"] = True
        else:
            st.write("ALTER TABLE query confirmed.")
        # If confirmed, execute the ALTER TABLE query.
        if st.session_state.get("alter_confirmed"):
            ...
            # Clear the pending ALTER TABLE query.
            st.session_state["pending_alter_query"] = None
            st.session_state["alter_confirmed"] = False

# --- Pending modification query handling (for non-ALTER modifications) ---
if st.session_state.get("pending_mod_query"):
    ...
    if not st.session_state.get("safe_mode", True):
        ...
        if st.session_state.get("modification_confirmed"):
            ...
            # Clear the pending modification query.
            st.session_state["pending_mod_query"] = None
            st.session_state["modification_confirmed"] = False
